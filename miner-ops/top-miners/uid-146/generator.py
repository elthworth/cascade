"""cascade-fullctx-research-v16 — mathematically refined full-context generator.
...
_SEASONAL_PROBS = np.array(
    [0.04, 0.12, 0.04, 0.02, 0.16, 0.05, 0.06, 0.04, 0.08, 0.03,
     0.07, 0.03, 0.13, 0.04, 0.03, 0.04, 0.06, 0.07, 0.04, 0.05],
    dtype=np.float64,
)
...
_FAMILIES: tuple[str, ...] = (
    ...
    "retail_demand",       # weekly retail profiles with latent drift and events
)
# The v16 dynamics-heavy base beat its prior mixture on all three controlled
# validation seeds (mean geomean 0.18431 vs 0.19097). A source-cluster-weighted
# audit of six lagged public eval reveals then found stable under-coverage of
# count, physical-sensor, and event mechanisms. This conservative experiment
# shifts only six percentage points toward those processes while retaining all
# tail families. It is an A/B candidate, not an asserted optimum.
...
class Generator(DataGenerator):
    ...
    @property
    def name(self) -> str:
        return str(self._cfg.get("name", "cascade-fullctx-research-v16"))
    ...
    def generate(self, n_series: int) -> Iterator[np.ndarray]:
        ...
        builders = (
            ...
            _seasonal_counts, _intermittent, _pulse_outlier, _retail_demand,
        )
                ...
                while produced < n_series and not stop.is_set():
                    ...
                    for fam in range(len(_FAMILIES)):
                        ...
                        if family == "retail_demand":
                            # This family deliberately mixes integer unit sales
                            # with continuous revenue/volume rows.
...
def _measurement_artifacts(
    ...
    *,
    ...
    if isinstance(preserve_integers, np.ndarray):
        ...
        out[integer_rows] = np.maximum(
            np.rint(out[integer_rows]), 0.0
        )
...
def _retail_demand(rng: np.random.Generator, n: int, L: int) -> np.ndarray:
    ...
    Adapted from the useful demand-specific branch in ``qweqwe_v18``. Unlike
    the generic seasonal-count family, each row has a seven-day profile,
    optional adjacent-day suppression, a slow latent log-demand walk, and
    short-lived promotions or stockouts. Rows mix integer counts with smooth
    non-negative magnitude series so the same latent retail law covers both
    unit sales and revenue/volume observations.
    """
    t = np.arange(L, dtype=np.float64)[None, :]
    tn = t / max(L - 1, 1)

    # A free seven-day profile captures arbitrary weekday effects. Most rows
    # also receive an adjacent two-day dip, approximating weekends or closures
    # without assuming that the week begins at a fixed phase.
    ...
    latent_walk = np.cumsum(
        rng.normal(0.0, 1.0, size=(n, L))
        * rng.uniform(0.005, 0.05, size=(n, 1)),
        axis=1,
    )
    np.clip(latent_walk, -3.0, 3.0, out=latent_walk)

    # Promotions have a one-step echo; stockouts are sharp negative events.
