...
def test_eval_audit_cadences_and_config_weights_are_active():
    module = _module()
    assert {15.0, 60.0, 240.0}.issubset(set(module._SEASONAL_PERIODS))
    assert np.all(module._SEASONAL_PROBS > 0.0)
    assert np.isclose(module._SEASONAL_PROBS.sum(), 1.0)

    config = json.loads((REPO / "config.json").read_text(encoding="utf-8"))
    configured = config["family_weights"]
    assert configured == module._DEFAULT_WEIGHTS
    assert np.isclose(sum(configured.values()), 1.0)
    assert configured["seasonal_counts"] >= 0.05
    assert configured["physical_sensors"] >= 0.04
    assert configured["pulse_outlier"] >= 0.02
