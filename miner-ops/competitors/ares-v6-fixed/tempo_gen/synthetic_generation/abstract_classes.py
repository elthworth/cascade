...
import numpy as np
# torch is NOT imported at module level: the trainer's generation sandbox
# rlimits the process to [generator] max_memory_mb (4096 MB) and importing
# torch there fails ("libtorch_cpu.so: failed to map segment"), killing the
# whole generator (mainnet failed_train). Only _set_random_seeds touches
# torch, and only if some family already imported it (gp/cauker); numpy-only
# family stacks never load it.
...
class GeneratorWrapper:
    ...
    def _set_random_seeds(self, seed: int) -> None:
        ...
        # Seed torch ONLY if a torch-using family already loaded it — importing
        # it here would OOM the sandbox rlimit for numpy-only family stacks.
        import sys as _sys
