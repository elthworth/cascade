"""cascade-fullctx-spectral-v2 — fast full-context mixture-of-priors generator.
...
intermittent demand, outliers) trains a stronger zero-shot model than the
reference generator's trend+seasonal+AR(1) mix.
...
# materialise more than one chunk. The base output working set is
# 1024 × 4096 × 8 B = 32 MiB across the emitted family blocks,
# plus temporary arrays. This remains comfortably below the 4 GiB sandbox cap
# and is materially faster than 256-row chunks under stream_cpu.
_CHUNK = 1024
...
# Broad near-equal composition is the robust default suggested by current
# corpus-mixture evidence. Seasonal structure remains the largest block, while
# every complementary family retains meaningful mass. These weights are an
# evidence-backed hypothesis for Toto2, not a claimed GPU A/B result.
_DEFAULT_WEIGHTS: dict[str, float] = {
    "trend_seasonal_ar": 0.18,
    "regime_shift": 0.12,
    "multiplicative": 0.12,
    "ar2": 0.10,
    "integrated": 0.08,
...
class Generator(DataGenerator):
    ...
    @property
    def name(self) -> str:
        return str(self._cfg.get("name", "cascade-fullctx-spectral-v2"))
    ...
    def generate(self, n_series: int) -> Iterator[np.ndarray]:
        ...
        builders = (
            ...
            _long_memory, _ou_stochastic_vol, _intermittent, _pulse_outlier,
        )
        produced = 0
...
def _trend_seasonal_ar(rng: np.random.Generator, n: int, L: int, *,
                       hi_frac: float = 0.25, exc_lo: float = 0.4, exc_hi: float = 3.0) -> np.ndarray:
    t = np.arange(L, dtype=np.float64)[None, :]
...
def _ou_stochastic_vol(rng: np.random.Generator, n: int, L: int) -> np.ndarray:
    ...
    switch_rate = np.exp(rng.uniform(np.log(0.001), np.log(0.03), size=(n, 1)))
    ...
    phi0 = rng.uniform(0.90, 0.99, size=(n, 1))
    phi1 = rng.uniform(0.995, 0.9995, size=(n, 1))
    phi = np.where(regime == 0, phi0, phi1)
    mu0 = rng.normal(-2.0, 1.0, size=(n, 1))
    mu1 = rng.normal(2.0, 1.0, size=(n, 1))
    mean = np.where(regime == 0, mu0, mu1)
    mean += _seasonal(rng, n, L, k_max=3) * rng.uniform(0.0, 1.5, size=(n, 1))
    ...
    heavy = rng.random((n, 1)) < 0.35
    student = rng.standard_t(4.0, size=(n, L)) / np.sqrt(2.0)
    eps = np.where(heavy, student, eps)
    shocks = rng.random((n, L)) < (3.0 / L)
    eps += shocks * rng.normal(0.0, 5.0, size=(n, L))

    out = np.empty((n, L), dtype=np.float64)
    out[:, 0] = mean[:, 0] + vol[:, 0] * eps[:, 0]
    for t in range(1, L):
        p = phi[:, t]
        innovation_scale = np.sqrt(np.maximum(1.0 - p * p, 1e-6))
        out[:, t] = (
            p * out[:, t - 1]
            + (1.0 - p) * mean[:, t]
            + innovation_scale * vol[:, t] * eps[:, t]
        )
