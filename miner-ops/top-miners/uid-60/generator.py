...
def _trend_seasonal_ar(rng: np.random.Generator, n: int, L: int) -> np.ndarray:
    ...
    slope = rng.normal(0.0, 0.8, size=(n, 1))                 # v4: total trend excursion, length-normalized
    series = level + slope * (t / max(L, 1)) + _seasonal(rng, n, L)
...
def _multiplicative(rng: np.random.Generator, n: int, L: int) -> np.ndarray:
    t = np.arange(L, dtype=np.float64)[None, :]
    growth = rng.normal(0.0, 0.5, size=(n, 1))                # v4: total log-growth, length-normalized
    base_level = np.exp(growth * (t / max(L, 1)) + rng.normal(0.0, 0.3, size=(n, 1)))  # positive, gentle
...
def _ar2(rng: np.random.Generator, n: int, L: int) -> np.ndarray:
    ...
    drift = rng.normal(0.0, 0.5, size=(n, 1)) * (np.arange(L, dtype=np.float64)[None, :] / max(L, 1))  # v4: length-normalized
    return x + drift


def _integrated(rng: np.random.Generator, n: int, L: int) -> np.ndarray:
    order2 = rng.random(n) < 0.35
    drift = rng.normal(0.0, 0.5, size=(n, 1)) / max(L, 1) ** 0.5   # v4: drift ~ sqrt(L) diffusion scale
