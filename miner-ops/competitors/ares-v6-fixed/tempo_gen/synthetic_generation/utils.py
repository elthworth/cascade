...
# torch intentionally NOT imported: the trainer's generation sandbox rlimits
# memory (4096 MB) and importing torch there dies with "libtorch_cpu.so:
# failed to map segment" == mainnet failed_train. generate_spikes only used
# torch.Tensor as a float32 container; _F32Array reproduces the exact
# quantization (and the caller's .numpy() call) without the import.
...
def generate_spikes(
    ...
    # torch.Tensor(x) cast to float32; .astype matches it byte-for-byte.
    return _F32Array(spikes.astype(np.float32))


class _F32Array(np.ndarray):
    """A float32 ndarray with torch.Tensor's ``.numpy()`` shim.

    Drop-in for the old ``torch.Tensor(spikes)`` return value: call sites do
    ``generate_spikes(...).numpy()`` — this keeps them working (returning the
    plain base ndarray) without torch in the import chain.
    """
