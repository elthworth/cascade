...
Six lagged public eval reveals (2026-07-16 through 2026-07-21) were then audited
by source cluster rather than raw series count. The stable gaps were missing
15/60/240-step cadences and low exposure to calendar counts, physical sensors,
and event processes. This candidate conservatively moves six percentage points
toward those mechanisms while retaining every tail family. Released values were
used only for structural coverage analysis, not as training data.
...
- Adds a 5% retail-demand branch adapted from `qweqwe_v18`, with free weekly
...
- Keeps independent outliers as only 15% of the 2%-weight pulse family; most
...
- Covers 15/60/240-step operational cadences as well as long
  365/672/730-step cycles, and retains a generic physical-sensor family
  (smooth, bounded, pressure-like, and skewed-positive) without copying
  source-specific trajectories.
...
later source-cluster audit supports a modest operational arm: seasonal counts
rise from 2% to 5%, physical sensors from 2% to 4%, and pulse processes from 1%
to 2%, funded by small reductions rather than deleting any family. These revised
weights have not yet been A/B scored; the prior screen remains directional
rather than a live-validator verdict.
...
precision. The source-balanced mixture should be treated as a candidate until
it beats the archived casade12 generator under the same seeds. Variable-length
training and stronger quantization/hold rates remain separate experiments;
combining them here would make any score change impossible to attribute.
