...
#
# REWEIGHT-ONLY v2 (custom-reweight-only): same 10 builders, mass tilted toward
# the families that most resemble v1's WEAK real-Toto2 domains, while keeping the
# strong seasonal engine (trend_seasonal_ar + multiplicative) dominant and no
# family near zero. Motivated per weak domain:
#   retail  (18%) -> intermittent  0.03 -> 0.06  (zero-inflated demand)
#   finance (45%) -> ar2 0.10->0.12, integrated 0.08->0.10  (near-unit-root/I(1))
#   econ    (40%) -> regime_shift  0.10 -> 0.12  (structural breaks)
#   sensor  (40%) -> chaotic 0.05->0.07, rff_gp 0.07->0.08  (nonlinear/smooth)
# Seasonal mass trimmed 0.42 -> 0.36 to fund the tilt (still the largest block,
# strong domains preserved). threshold_ar / pulse_outlier held to keep diversity.
_DEFAULT_WEIGHTS: dict[str, float] = {
    "trend_seasonal_ar": 0.22,
    "regime_shift": 0.12,
    "multiplicative": 0.14,
    "ar2": 0.12,
    "integrated": 0.10,
...
class Generator(DataGenerator):
    ...
    @property
    def name(self) -> str:
        return str(self._cfg.get("name", "custom-mixture-of-priors-v2"))
    ...
    def generate(self, n_series: int) -> Iterator[np.ndarray]:
        ...
        builders = (
            ...
            _ar2, _integrated, _threshold_ar, _chaotic, _rff_gp,
